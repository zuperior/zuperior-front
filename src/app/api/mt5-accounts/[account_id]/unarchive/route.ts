// client/src/app/api/mt5-accounts/[account_id]/unarchive/route.ts
...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/mt5/unarchive-account/${account_id}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: errorData.detail || errorData.message || 'Failed to unarchive account'
        },
